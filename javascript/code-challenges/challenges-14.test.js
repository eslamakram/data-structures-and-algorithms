...
Write a function named screenForNames that takes in an array of
 strings and uses Regex to create a new array of only those strings
...
let biggerThanLuke = (arr) => {
  return arr.filter(value => (value.mass > 77)).map(value => value.name).join(' - ');};
...
const isSecure = (url) => {
  let x ;
  url.slice(0,5).includes('s') && url.includes('//')? x= true : x=false 
      return x;
  };
