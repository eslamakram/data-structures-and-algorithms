...
const longestString = (arr) => {
  let max ='';
  arr.map(item=>item.length>max.length?max=item:'');
...
const findHappiness = (arr) => {
  // Solution code here...
  let result =[]
  arr.map(item=>item.includes(':)')&&result.push(item))
  return result
};
...
const standardizePhoneNumbers = (arr) => {
  let result=[]
  arr.map(item=>result.push(`${item.substring(1, 4)}${item.substring(6, 9)}${item.substring(10)}`))
  return result
}
...
const onlyOddChars = (str) => {
  let result=[...str]
  let newstring=[]
  result.map(item=>result.indexOf(item)%2===1&&newstring.push(item))
  return(newstring.join(''));};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 6 

Write a function named allHappy that takes in an array of strings and returns 
a Boolean indicating whether all those strings contain ":)".
------------------------------------------------------------------------------------------------ */

const allHappy = (arr) => {
  let result = []
  arr.map(item=>result.push(item.includes(':)')))
